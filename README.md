...
Projeto desenvolvido no módulo de backend do curso de Desenvolvimento Web da Trybe. O projeto MongoDB Aggregations foi o projeto de fechamento do bloco de estudos sobre bancos de dados NoSQL e MongoDB. Ao longo desta etapa, me familiarizei com diversos métodos e operadores para execução de operações de agregação de dados no **MongoDB** e coloquei em prática estes aprendizados neste projeto. Para este projeto, diferente de outros projetos anteriores, foram utilizadas diversas collections, com o objetivo de praticar em diferentes cenários e utilizar diversos operadores e estágios do `aggregation pipeline`. 
Portanto, neste projeto, trabalhei com dados do **IMDB**, sobre empresas aéreas e também dados com registros de deslocamento de pessoas que utilizam bikes. 
...
- Execução de buscas complexas no banco mongoDB
- Utilização dos operadores de aggregation para criação de uma pipeline
